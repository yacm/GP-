...
for j in range(0,nu.shape[1]):
    x_gri0,V0,Y0,Gamma0 = preparedata(j,nu,rMj,rMe,rM,scale="log")
    myGP0= GaussianProcess(x_gri0,V0,Y0,Gamma0,f"z={j+1}a",flag="noisy",device="cpu",Pd=simplePDFnormed, Ker=Krbf1,Pd_args=(-0.0,1.0,1.0),Ker_args=(0.477,-0.80,0.05))
    myGP0.prior2ndlevel("all",1000,mean=tr.tensor([-1.0,0.0,0.001,-1.0,-1.5,-5.0]),sigma=tr.tensor([2.0,6.0,2.0,5.0,1.2,10.0]),prior_mode=tr.tensor([2,2,2,2,2,2]))
    fits_comb.append(myGP0)
    print(fits_comb[j].name, "done")
x_gri0,V0,Y0,Gamma0 = preparemockdata(7)
myGP0= GaussianProcess(x_gri0,V0,Y0,Gamma0,f"mock-data",flag="noisy",device="cpu",Pd=simplePDFnormed, Ker=Krbf1,Pd_args=(-0.0,1.0,1.0),Ker_args=(0.477,-0.80,0.05))
myGP0.prior2ndlevel("all",1000,mean=tr.tensor([-1.0,0.0,0.001,-1.0,-1.5,-5.0]),sigma=tr.tensor([2.0,5.0,2.0,5.0,1.2,10.0]),prior_mode=tr.tensor([2,2,2,2,2,2]))
fits_comb.append(myGP0)
print(fits_comb[-1].name, "done")

#train to accelerate the process
mode='all'
if i in [1,4]:
    fits_comb[i].train(3000,lr=1e-2,mode=mode,function="nlp")
elif i in [12]:
    fits_comb[i].train(3000,lr=1e-2,mode=mode,function="nlp")
else:
    fits_comb[i].train(3000,lr=1e-2,mode=mode,function="nlp")
...
##sample kernel
momentum=1*tr.tensor([1.0,10.0,1.0,1.0,1.0,1.0])
samplers=[]
for i in range(0,13):

    #fits_comb[i].mode="kernel"
    GPsampler=HMC_sampler(fits_comb[i].nlogpost2levelpdf,device="cpu",diagonal=1*tr.ones(6),grad=None)
...
traceq,tracep,traceH=samplers[i].sample(samplers[i].q0,Nsamples,eps,L)
print(samplers[i].q0,samplers[i].eps,samplers[i].L,samplers[i].diagonal)
tr.save(traceq,'Krbf1(%s).pt' %(fits_comb[i].name))
